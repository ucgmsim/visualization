...
def top_template():
    ...
    # fault path
    if args.srf is not None:
        t.fault(args.srf, is_srf=True, plane_width=0.5, \
...
def render_slice(n):
    ...
    if args.seis is not None:
        ...
        s.seismo(os.path.abspath(args.seis), n, \
                 fmt='time', \
                 colour='red', \
                 width='1p')
...
gmt.make_movie('%s/ts%%04d.png' % (png_dir), args.output, \
               fps=int(1.0/xyts.dt * args.scale), codec='libx264')
print('finished.')
# cleanup
rmtree(gmt_temp)
