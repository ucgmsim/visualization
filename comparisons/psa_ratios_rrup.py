...
name_rrup = np.loadtxt(args.stats, dtype='|S7,f', \
...
for im in im_names:
    ...
    im_ratios = np.log(obs_ims[im][obs_idx].tolist()) - \
                np.log(sim_ims[im][sim_idx].tolist())
    bias_string = 'median=%.2g, sigma=%.2g' % (np.mean(im_ratios), \
                                               np.std(im_ratios, ddof=1))

    # plot
    fig = plt.figure(figsize = (14, 7.5), dpi = 100)
    plt.rcParams.update({'font.size': 18})
    plt.semilogx(name_rrup['f1'], im_ratios, linestyle='None', \
                 color='blue', marker='s', markersize=12, label=bias_string)
