...
def get_ims_psa(arg_sim):
    ...
    sim_psa = [sim_ims.dtype.names[col_i] for col_i in np.where(np.logical_and(np_startswith(sim_ims.dtype.names, 'pSA_'), np.logical_not(np_endswith(sim_ims.dtype.names, 'sigma'))))[0]]
