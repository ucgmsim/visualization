...
name_rrup = np.loadtxt(
    args.rrup, dtype="|S7,f", usecols=(0, 3), skiprows=1, delimiter=","
)
...
if args.srf is not None:
    ...
    r_rup_vals = np.exp(
        np.linspace(np.log(args.dist_min), np.log(args.dist_max), args.n_val)
    )
...
for im in im_names:
    ...
    plt.savefig(
        os.path.join(args.out_dir, "%s_with_Rrup_%s.png" % (print_name, args.run_name))
    )
    plt.close()
