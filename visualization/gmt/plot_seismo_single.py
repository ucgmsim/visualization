...
xy = (
    str(np.dstack((time, series)).tolist())
    .replace("], [", "\n")
    .rstrip("]")
    .lstrip("[")
    .split("\n")
...
t.spacial("X", (0, max_t, -80, 80), sizing="10/2", x_shift=1, y_shift=1)
...
def render(i):
    ...
    p.path("\n".join(xy[: 20 * i]), is_file=False, colour="white", width=1.2)
    p.finalise()
