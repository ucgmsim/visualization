...
for seg in xrange(len(bounds)):
    ...
    tinit[seg].astype(np.float32).tofile("%s/PLANES/tinit_map_%d.bin" % (out_dir, seg))
...
zoom_width, zoom_height = map_width("M", full_height, plot_region, wd=out_dir)
...
while ((plot_region[1] - plot_region[0]) / major_tick) / zoom_width > 1.4:
    major_tick += 0.05
p.ticks(major="%sd" % (major_tick), minor="%sd" % (major_tick / 2.0), sides="ws")
...
p.fault(srf, is_srf=True, plane_width="0.4p", top_width="0.6p", hyp_colour="red")
...
# scale
p.cpt_scale(
    zoom_width / 2.0, -0.5, "ginc.cpt", label="Slip (cm)", length=zoom_width, log=True
)
