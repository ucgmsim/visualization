...
for i, f in enumerate(srf_files):
    ...
    #    continue
    try:
        srf_data.append(gmt.srf2map(f, gmt_temp, prefix=good_srf, wd=gmt_temp))
...
ni_width = 9
ni_height = gmt.mapproject(
    ni_region[0],
    ni_region[3],
    wd=gmt_temp,
    projection="M%s" % (ni_width),
    region=ni_region,
)[1]
...
def teleport(plot, location):
    ...
    if location == "ni":
        plot.spacial("M", ni_region, sizing=ni_width, x_shift=0)  # si_width + i_gap)
...
for i in ["ni"]:
    ...
    if i == "si" or i == "ni":
        ...
        for s in range(len(srf_data)):
            for plane in range(len(srf_data[s][1])):
                p.overlay(
...
# scale
p.cpt_scale(
    (ni_width + si_width + i_gap) / 2.0,
    -0.5,
    "ginc0.cpt",
    label="Slip (cm)",
    length=si_width + ni_width + i_gap,
    log=False,
)
