...
def prepare(sd, tp):
    ...
    srf2cnr = lambda path: "%s/cnrs.txt" % (os.path.basename(os.path.splitext(path)[0]))
    ...
    for srf in srf_files:
        ...
        os.remove(corners)
        tar.add(
            "%s/LF/%s/OutBin/%s_xyts.e3d" % (sd, srf_name[:-4], run_name),
...
if sys.argv[1] == "prepare":
    ...
    Popen(["ssh", "%s@fitzroy.nesi.org.nz" % (ruser), "mkdir -p %s" % (rtemp)]).wait()
    # everything is put in same location, no need for qcore_path.py
    Popen(
        ["ssh", "%s@fitzroy.nesi.org.nz" % (ruser), "touch %s/qcore_path.py" % (rtemp)]
    ).wait()
    ...
    Popen(
        [
            ...
            "%s/../srf.py" % (lbase),
            "%s/../geo.py" % (lbase),
            "%s@fitzroy.nesi.org.nz:%s/" % (ruser, rtemp),
        ]
    ).wait()
    # run prepare in temp
    ...
    Popen(
        [
            "ssh",
            "%s@fitzroy.nesi.org.nz" % (ruser),
            "/opt/niwa/Python/AIX/2.7.5/bin/python %s/%s prepare %s %s"
            % (rtemp, os.path.basename(__file__), sys.argv[2], "%s/data.tar" % (rtemp)),
        ]
    ).wait()
    ...
    Popen(["ssh", "%s@fitzroy.nesi.org.nz" % (ruser), "rm -r %s" % (rtemp)]).wait()
    # local tar file
    ...
    if not os.path.exists("%s/params_plot.py" % (wd)):
        copyfile("%s/params_plot.template.py" % (lbase), "%s/params_plot.py" % (wd))
    # start PGV/MMI plot
