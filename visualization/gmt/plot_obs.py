...
if not os.path.exists(obs_velbb):
    ...
    exit(1)
event_stats = os.path.join(base_dir, "Stat", event_name, "%s.ll" % (event_name))
...
lon_size, lat_size = gmt.mapproject(
    x_max, y_max, wd=seisplot.wd, projection="M%s" % (seisplot.width), region=ll_region
)
if seisplot.min_dist == None:
    seisplot.min_dist = lon_dist / 12.0
...
for s, stat in enumerate(stats):
    x0, y0 = gmt.mapproject(
        lons[s],
        lats[s],
        wd=seisplot.wd,
        projection="M%s" % (seisplot.width),
        region=ll_region,
    )
...
b.background(11, 11)
b.spacial("M", ll_region, sizing=seisplot.width, x_shift=1, y_shift=1)
...
b.basemap()
# stations
b.points(event_stats, shape="t", size=0.08, fill=None, line="white", line_thickness=0.8)
b.points(
    plot_stats, shape="t", size=0.08, fill="blue", line="white", line_thickness=0.8
)
...
if SIM_DIR or sfs_modelparams != None:
    # simulation domain
    b.path(cnr_str, is_file=False, split="-", close=True, width="0.4p", colour="black")
...
if seisplot.obs_src != None:
    b.seismo(
        ...
        seisplot.max_ts,
        fmt="inc",
        width=seisplot.seis_width,
        colour=seisplot.obs_colour,
    )
