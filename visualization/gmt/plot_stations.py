...
def load_file(station_file):
    # load values
    val_pool = np.atleast_2d(np.loadtxt(station_file, dtype="f", skiprows=6)[:, 2:].T)
    ...
    with open(station_file) as statf:
        ...
        if len(cpt_info2) > 1:
            ...
            for i in xrange(len(cpt_max)):
                if cpt_max[i] > 115:
                    ...
                    cpt_max[i] = round(
                        cpt_max[i], 1 - int(np.floor(np.log10(abs(cpt_max[i]))))
                    )
                else:
                    # 1 significant figures
                    cpt_max[i] = round(
                        cpt_max[i], -int(np.floor(np.log10(abs(cpt_max[i]))))
                    )
                if val_pool[i].min() < 0:
                    cpt_min.append(-cpt_max[i])
                else:
                    cpt_min.append(0)
                cpt_inc.append(cpt_max[i] / 10.0)
                cpt_tick.append(cpt_inc[i] * 2.0)
        if len(cpt_info2) > 2:
        ...
        if len(col_labels) != ncol and len(col_labels) != 0:
            print(
                "%d column labels found when there are %d columns."
                % (len(col_labels), ncol)
            )
            exit(1)
...
def determine_sizing(args, meta):
    ...
    if args.region is not None:
        ...
        # fit all values
        xy = np.loadtxt(args.station_file, skiprows=6, usecols=(0, 1), dtype="f")
    ...
    if meta["grid"] is not None:
        ...
        gmt.grd_mask(
            "%s/sim.modelpath_hr" % (meta["gmt_temp"]),
            "%s/mask.grd" % (meta["gmt_temp"]),
            dx=meta["stat_size"],
            dy=meta["stat_size"],
            region=meta["region"],
        )
    ...
    if args.tick_major is None:
        ...
        args.tick_minor = args.tick_major / 5.0
...
def template(args, meta):
    ...
    t.background(11, 15)
    t.spacial("M", meta["region"], sizing=args.width, x_shift=1, y_shift=2.5)
...
def column_overlay(args_meta_n):
    ...
    if "categorical" in meta["cpt_properties"]:
        ...
        p.cpt_scale(
            ...
            meta["cpt_inc"][n],
            label=meta["legend"],
            arrow_f=meta["cpt_max"][n] > 0,
            arrow_b=meta["cpt_min"][n] < 0,
        )

    # fault planes
    if os.path.exists("%s/srf_cnrs.txt" % (meta["gmt_temp"])):
        p.fault(
            "%s/srf_cnrs.txt" % (meta["gmt_temp"]),
            is_srf=False,
            plane_width=0.5,
            top_width=1,
            hyp_width=0.5,
            top_colour=meta["overlays"],
            plane_colour=meta["overlays"],
            hyp_colour=meta["overlays"],
        )
    p.leave()
    ...
    p.png(
        dpi=args.dpi,
        clip=True,
        out_name=os.path.join(args.out_dir, os.path.splitext(os.path.basename(top))[0]),
    )
...
def load_args():
    ...
    parser.add_argument(
        "--out_dir", help="folder where outputs are saved", default="PNG_stations"
    )
    parser.add_argument(
        "--srf", help="srf file, will use corners file instead if available"
    )
    parser.add_argument("--srf_cnrs", help="standard srf fault corners file")
    parser.add_argument(
        "--model_params", help="model_params file for simulation domain"
    )
    parser.add_argument(
        "--region", help="plot region. xmin xmax ymin ymax.", type=float, nargs=4
    )
    parser.add_argument(
        "-n",
        "--nproc",
    ...
    parser.add_argument(
        "--width", help="map width in default units", type=float, default=6.0
    )
    parser.add_argument("--tick_major", help="major map tick increment", type=float)
    parser.add_argument("--tick_minor", help="minor map tick increment", type=float)
    parser.add_argument("--sites", help="locations to label", default="auto")
    parser.add_argument("--dpi", help="output resolution", type=int, default=300)
