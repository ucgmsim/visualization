...
# TODO: change all references to hardcoded values and extract them to config (ie. /nesi/project/nesi00213/).
...
faults_historic = [
    ...
    "23Deca2011FaultPlane.xy",
    "23Decb2011FaultPlane.xy",
    "20161114 Kaikoura Ian02_s103245.txt",
    "Cook Strait 012017.txt",
    "DarfieldFaultPlane.xy",
    "Wilberforce 122016.txt",
    "2014Jan20_Eketahuna_m6p3.txt",
]
faults_future = [
    "Alpine Fault m7.90-411.0x17.3_s1129570.txt",
    "Porters Pass M7.20_82.0by14.0_s83747.txt",
]
srf_files = [
    "20161114 Kaikoura Ian02_s103245.srf",
    "20110222 m6p2.srf",
    "20100904 m7p1.srf",
...
si_width = 6
si_height = gmt.mapproject(
    si_region[0],
    si_region[3],
    wd=gmt_temp,
    projection="M%s" % (si_width),
    region=si_region,
)[1]
...
ni_width, ni_height = gmt.map_width(
    "M", si_height, ni_region, wd=gmt_temp, accuracy=0.001
)
# functions to go back and forth between plots
def teleport(plot, location):
    if location == "si":
        plot.spacial("M", si_region, sizing=si_width, x_shift=-si_width - i_gap)
    elif location == "ni":
        plot.spacial("M", ni_region, sizing=ni_width, x_shift=si_width + i_gap)
...
# add title
p.text(
    left_margin + (si_width + i_gap + ni_width) / 2,
...
for i in ["ni", "si"]:
    ...
    if i == "si" or i == "ni":
        ...
        for s in xrange(len(srf_data)):
            for plane in xrange(len(srf_data[s][1])):
                p.overlay(
                    "%s/%d_%d_slip.bin" % (gmt_temp, s, plane),
                    "%s/%d.cpt" % (gmt_temp, s),
                    dx=srf_data[s][0][0],
                    dy=srf_data[s][0][1],
                    climit=2,
    ...
    for label in labels:
        if region[0] <= label[0] < region[1] and region[2] <= label[1] <= region[3]:
            p.text(
                label[0],
                label[1],
                label[3],
