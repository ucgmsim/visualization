...
if scenarios > 1:
    ...
    xyts.pgv(
        mmi=True, pgvout="%s/PGV.bin" % (gmt_temp), mmiout="%s/MMI.bin" % (gmt_temp)
    )
...
if scenarios == 1:
    suffixes = [None]
    np.savetxt("%s/nztm2000pgv.txt" % (png_dir), xyv, fmt="%f", delimiter=",")
else:
    suffixes = ["%.3d" % (i) for i in xrange(scenarios)]
    suffixes.extend(["MIN", "MAX"])
    for suffix in suffixes:
        xyv[:, 2] = np.fromfile("%s/PGV_%s.bin" % (gmt_temp, suffix), dtype="3f")[:, 2]
        np.savetxt(
            "%s/nztm2000pgv_%s.txt" % (png_dir, suffix), xyv, fmt="%f", delimiter=","
        )
...
gmt.grd_mask("%s/sim.modelpath_hr" % (gmt_temp), mask, dx=dx, dy=dy, region=plot_region)
...
for s in xrange(len(suffixes)):
    ...
    pager_grid.write_shakemap_grid_header(
        event_id,
        event_type,
        mag,
        dep,
...
if pgvplot.major_tick == None:
    ...
    pgvplot.major_tick, pgvplot.minor_tick = gmt.auto_tick(x_min, x_max, width)
elif pgvplot.minor_tick == None:
    pgvplot.minor_tick = pgvplot.major_tick / 5.0
# colour palettes
...
if pgvplot.cpt_inc == None or pgvplot.cpt_max == None:
    pgvplot.cpt_inc, pgvplot.cpt_max = gmt.xyv_cpt_range(
        reference, my_inc=pgvplot.cpt_inc, my_max=pgvplot.cpt_max
    )[1:3]
...
p.background(11, 11)
p.spacial("M", plot_region, sizing=pgvplot.width, x_shift=1, y_shift=2.5)
# common titles
...
for s in xrange(len(suffixes)):
    ...
    p.overlay(
        ...
        dx=dx,
        dy=dy,
        climit=pgvplot.convergence_limit,
        min_v=pgvplot.lowcut,
        crop_grd=mask,
        contours=pgvplot.cpt_inc,
        land_crop=pgvplot.land_crop,
    )
    m.overlay(
        mmi_in,
        gmt.CPTS["mmi"],
        dx=dx,
        dy=dy,
        climit=mmiplot.convergence_limit,
        min_v=0,
        crop_grd=mask,
        contours=1,
        land_crop=mmiplot.land_crop,
    )
    p.sites(plot_sites)
    m.sites(plot_sites)
    if scenarios == 1:
        p.fault(srf_cnrs[0], is_srf=False, plane_width=0.5, top_width=1, hyp_width=0.5)
        m.fault(srf_cnrs[0], is_srf=False, plane_width=0.5, top_width=1, hyp_width=0.5)
    p.coastlines()
    m.coastlines()
    ...
    p.cpt_scale(
        ...
        pgvplot.cpt_inc,
        pgvplot.cpt_inc,
        label=pgvplot.cpt_legend,
        arrow_f=pgvplot.cpt_max > 0,
        arrow_b=pgvplot.cpt_min < 0,
    )
    m.cpt_scale(3, -0.5, gmt.CPTS["mmi"], 1, 1, label=mmiplot.cpt_legend)
    p.ticks(major=pgvplot.major_tick, minor=pgvplot.minor_tick)
    m.ticks(major=pgvplot.major_tick, minor=pgvplot.minor_tick)
    p.path(cnr_str, is_file=False, split="-", close=True, width="0.4p", colour="black")
    m.path(cnr_str, is_file=False, split="-", close=True, width="0.4p", colour="black")
