...
def kminch_scale():
    ...
    plot_y = (media[1] - (nrow - 1) * row_spacing - top_margin - bottom_margin) / nrow
    ...
    plot_x_total = (
        left_margin + (ncol - 1) * col_spacing + sum(seg_len) / km_inch + right_margin
    )
...
scale_factor = 1.0 / max(log(km_inch), 2)
base_size = scale_factor * 24
annot_size = 4.0 / 5.0 * base_size
label_size = 4.0 / 3.0 * base_size
...
rakes = srf.srf2llv_py(
    args.srf_file, join_minor=True, value="rake", flip_rake=True, lonlat=False
)
...
for i, values in enumerate([slip_values, trise_values]):
    ...
    gmt.makecpt(
        plot_cpt[i], cpt_out[i], 0, cpt_max[i], cpt_max[i] / 1000, continuous=True
    )
...
for s, seg in enumerate(planes):
    for r, row_data in enumerate([slips[s], trises[s], rakes[s]]):
        ...
        if s == 0 and r == 0:
            ...
            y_shift = (
                media[1] - top_margin - max(seg_wid_d) + (max(seg_wid_d) - seg_wid_d[s])
            )
        elif r != 0:
            x_shift = 0
            ...
            y_shift = (
                (nrow - 1) * (row_spacing + max(seg_wid_d))
        ...
        if r != 2:
            ...
            p.overlay(grd_file, cpt_out[r], dx=srf_dx, dy=srf_dy, transparency=0)
        else:
            # rake angles
        ...
        if r == 0:
            ...
            p.overlay(
                ...
                dy=srf_dy,
                acontours=acontour,
                font_size=annot_size,
                contour_thickness=scale_factor,
        ...
        p.cpt_scale(
            ...
            dx=col_spacing,
            pos="rel_out",
            cross_tick=cpt_max[r] / 4.0,
            length=max(seg_wid_d),
            thickness=scale_factor / 5.0,
            horiz=False,
            align="LT",
            label=labels[r],
        )
...
if args.title is not None:
    p.text(
        (sum(seg_len) + col_spacing * (ncol - 1) * km_inch) / -2 + seg_len[-1],
